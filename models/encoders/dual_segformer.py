...
class MultiScaleAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, local_region_shape = [4, 8, 40]):
        ...
        weights = torch.linspace(0.1, 1, len(local_region_shape)+1) 
        weights = torch.cat((weights[-1:], weights[:-1]), dim=0)
        
        self.weights = nn.Parameter(weights) # declare as a model parameter
