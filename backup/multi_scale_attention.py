...
class MultiScaleAttention(nn.Module):
    ...
    def attention(self, q, k, v):
        try:
            ...
            print(f'********************* excp occurs:::q:{q.shape} k:{k.shape} scale:{self.scale} sr:{self.sr_ratio} numh:{self.num_heads} head_dim:{self.head_dim} ')
            exit()
    ...
    def forward(self, x, H, W):
        ...
        print(f'self sr ratio:{self.sr_ratio} ')
        if self.sr_ratio > 1:       
